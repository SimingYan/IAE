...
from Torch_Utility import copy_parameters, copy_parameters_ft
...
def main(args):
    ...
    nparameters = sum(p.numel() for p in classifier.parameters())
    ...
    if args.restore:
        ...
        classifier = copy_parameters_ft(classifier, model_dict, verbose=True)
    ...
    for epoch in range(MyLogger.epoch, args.epoch + 1):
        ...
        for points, target in tqdm(trainDataLoader, total=len(trainDataLoader), smoothing=0.9):
            writer.add_scalar('Learning_Rate', scheduler.get_lr()[-1], MyLogger.step)
